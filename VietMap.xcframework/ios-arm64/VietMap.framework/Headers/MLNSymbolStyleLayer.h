...
typedef NS_ENUM(NSUInteger, MLNSymbolZOrder) {
    /**
     If `MLNSymbolStyleLayer.symbolSortKey` is set, sort based on that.
...
/**
 An `MLNSymbolStyleLayer` is a style layer that renders icon and text labels at
 points or along lines on the map.
 
 Use a symbol style layer to configure the visual appearance of feature labels.
 These features can come from vector tiles loaded by an `MLNVectorTileSource`
 object, or they can be `MLNShape` or `MLNFeature` instances in an
 `MLNShapeSource` or `MLNComputedShapeSource` object.
...
/**
 ...
 Use the `+[MLNStyle setImage:forName:]` method to associate an image with a
...
/**
 ...
 `symbolPlacement` property is set to `MLNSymbolPlacementPoint` but this layer
 is above a symbol layer whose `symbolPlacement` property is set to
 `MLNSymbolPlacementLine`. You do not need to enable this property to prevent
